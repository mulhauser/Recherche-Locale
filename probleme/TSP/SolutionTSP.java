...
public class SolutionTSP extends SolutionAbstract {
     ...
     *
     * @param s solution dont on construit une copie
     */
     ...
     *
     * @param i indice ville 1 a inverser
     * @param j indice ville 2 a inverser
     */
    ...
    public List<SolutionAbstract> retourneVoisinage() {
        ...
        ArrayList<Ville> listeVillesTemp = listeVilles;

        for (int i = 0; i < listeVilles.size(); i++) {
            for (int j = i; j < listeVilles.size(); j++) {
                if (listeVilles.get(i).idVille != listeVilles.get(j).idVille) {
                    inverse(i, j);
                    SolutionTSP nouvelleSolution;
                    nouvelleSolution = new SolutionTSP(this);
